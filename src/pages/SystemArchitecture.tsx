...
import { Cpu, Activity, UserCheck, Brain, Film, FileText, ArrowDown } from 'lucide-react';
  ...
  const levels = [
    {
      ...
      model: 'Custom Dual-Branch Hybrid ConvNet',
      technique: '2D Fast Fourier Transform (FFT) + Spatial Conv2D',
      desc: 'Converts input to Grayscale, resizes to 32x32. Branch A computes spatial Conv2D features. Branch B computes 2D Fast Fourier Transform (torch.fft.fft2), shifts spectrum (fftshift), takes log-magnitude magnitude (torch.log1p), and passes through Conv2D. Features concatenate into a 256-channel classifier.'
    },
    {
      level: 'Level 2',
      title: 'Facial Integrity & Deepfake Expert',
      model: 'MobileNetV2 Deep CNN + OpenCV Cascade',
    ...
    {
      level: 'Level 4',
      title: 'Video Temporal Aggregation Expert',
      model: 'Frame Sampling & Ensemble Aggregator',
      technique: 'Uniform Keyframe Step Sampling (15-20 frames)',
      desc: 'Extracts 15 to 20 keyframes across video duration using OpenCV cv2.VideoCapture. Passes keyframes through Level 1, 2, 3 experts and computes mean and peak temporal risk scores.'
    },
    {
      level: 'Level 5',
    ...
    <div className="space-y-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">
              Hierarchical 5-Layer Forensic Domain Architecture & Weighted Ensemble Math
            </p>
          </div>
        </div>

        <p className="text-xs text-slate-300 leading-relaxed">
          The engine replaces monolithic classifiers with a multi-level expert ensemble. Inputs undergo simultaneous forensic inspection across 4 physical and structural domain perspectives before an ensemble fusion engine aggregates scores into a final verdict.
        </p>
      </div>
      ...
      <div className="space-y-4">
        <h4 className="text-sm font-bold text-slate-300 uppercase tracking-wider">
          🧠 Multi-Level Neural Expert Levels
        </h4>
